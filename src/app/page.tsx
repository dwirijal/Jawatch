...
export default async function HomePage() {
  const response = await getContents(undefined, 30);
  ...
  return (
    <div className="min-h-screen">
            ...
            <div className="max-w-2xl animate-fade-in-up">
              ...
              <div className="inline-flex items-center gap-2 mb-6">
                ...
                <span className="px-3 py-1 bg-white/10 backdrop-blur-sm text-white text-xs font-semibold uppercase tracking-wider rounded">
                  {heroContent.content_type === 'anime' ? 'Anime' : 'Manga'}
                </span>
              ...
              <div className="flex items-center gap-4 mb-6 text-gray-300">
                <span className="text-sm font-semibold">
                  {new Date(heroContent.scraped_at).getFullYear()}
                </span>
                <span className="w-1 h-1 bg-gray-500 rounded-full"></span>
                <span className="text-sm">
                  {heroContent.content_type === 'anime' ? 'Streaming' : 'Reading'}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                <Link
                  href={heroContent.content_type === 'anime' ? `/watch/${heroContent.id}` : `/read/${heroContent.id}`}
                ...
                >
                  ...
                  {heroContent.content_type === 'anime' ? 'Watch Now' : 'Read Now'}
                </Link>
                <Link
                  href={heroContent.content_type === 'anime' ? `/watch/${heroContent.id}` : `/read/${heroContent.id}`}
      ...
      {/* Anime Section - Netflix Row Style */}
      {anime.length > 0 && (
        <section className="mb-16 animate-slide-in-right">
          <div className="px-8 md:px-16 lg:px-24 mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 mt-2">Streaming now</p>
            </div>
          ...
          <div className="content-row flex gap-4 overflow-x-auto pb-4 px-8 md:px-16 lg:px-24 snap-x snap-mandatory">
            {anime.slice(0, 15).map((item, index) => (
              <Link
                key={item.id}
                href={`/watch/${item.id}`}
      ...
      {/* Manga Section - Webtoon Grid Style */}
      {manga.length > 0 && (
        <section className="mb-16 animate-slide-in-right">
          <div className="px-8 md:px-16 lg:px-24 mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 mt-2">New chapters available</p>
            </div>
          </div>

          <div className="px-8 md:px-16 lg:px-24">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {manga.slice(0, 15).map((item, index) => (
                <Link
                  key={item.id}
                  href={`/read/${item.id}`}
