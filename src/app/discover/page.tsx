  ...
  return (
    <div className="mx-auto max-w-[1160px] px-4 py-12 sm:px-8">
      ...
      <div className="mb-8 flex flex-wrap gap-2">
        {types.map((type) => (
          <Link key={type} href={`/discover/${type}`} className="rounded-full border border-border bg-card px-4 py-2 font-mono text-[11px] uppercase tracking-tag text-muted-foreground transition-colors hover:border-primary hover:text-primary">
            {type}
          </Link>
        ))}
      </div>
