...
export function HeroSpotlight({ item }: HeroSpotlightProps) {
  ...
  return (
    <section className="relative min-h-[82vh] overflow-hidden border-b border-hairline bg-void grain">
      {item.coverImage && (
      ...
      <div className="relative z-10 mx-auto grid min-h-[82vh] w-full max-w-[1160px] items-end gap-10 px-4 pb-16 pt-28 sm:px-8 md:grid-cols-[minmax(0,1fr)_300px] md:pb-24">
        <div className="max-w-3xl">
          <div className="mb-5 flex flex-wrap items-center gap-3">
            <span className="rounded-sm border border-amber/60 px-2 py-1 font-mono text-[10px] uppercase tracking-[.14em] text-amber">{item.type}</span>
            {item.rating?.average ? (
          ...
          <h1 className="max-w-4xl font-serif text-5xl font-semibold leading-none tracking-tight text-paper text-shadow-lg sm:text-7xl">{item.title}</h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href={primaryHref} className="inline-flex items-center gap-2 rounded-[4px] bg-amber px-6 py-3 font-mono text-xs font-semibold uppercase tracking-[.08em] text-void transition-colors hover:bg-amber/90">
              {isReadable ? <BookOpen className="h-4 w-4" aria-hidden="true" /> : <Play className="h-4 w-4 fill-void" aria-hidden="true" />}
              Start watching
            </Link>
            <Link href={primaryHref} className="inline-flex items-center gap-2 border border-hairline bg-surface/75 px-[26px] py-[13px] font-mono text-xs uppercase tracking-[.06em] text-paper transition-colors hover:border-amber/60 hover:text-amber">
        ...
        {item.coverImage && (
          <div className="relative hidden aspect-[2/3] overflow-hidden rounded-[4px] border border-amber/20 bg-surface shadow-2xl shadow-amber/5 md:block">
