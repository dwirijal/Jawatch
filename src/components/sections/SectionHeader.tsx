...
export function SectionHeader({ eyebrow, title, description, href, actionLabel = 'View all' }: SectionHeaderProps) {
  return (
    <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between border-l-2 border-amber pl-4">
      <div>
        {eyebrow && <div className="font-mono text-[9px] uppercase tracking-eyebrow text-accent-bright">{eyebrow}</div>}
      ...
      {href && (
        <Link href={href} className="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-label text-primary transition-colors hover:text-primary/80 min-h-[48px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-2 sm:p-0">
          {actionLabel}
