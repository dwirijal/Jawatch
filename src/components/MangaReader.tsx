...
export function MangaReader({ slug, chapters, initialPages, currentChapterSlug }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Chapter header */}
      <div className="flex items-center justify-between gap-2 border border-hairline bg-surface/30 p-4 grain">
        <h2 className="text-sm font-mono uppercase tracking-wider text-paper">
          Chapter {chapters[chIndex]?.chapterNumber || chIndex + 1}
        </h2>
        <button
          onClick={() => setShowList(!showList)}
          className="px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider border border-hairline hover:border-amber/60 text-paper transition-all"
        >
      ...
      {/* Chapter dropdown */}
      {showList && (
        <div className="max-h-60 overflow-y-auto bg-surface/50 border border-hairline p-2 grid grid-cols-2 sm:grid-cols-3 gap-1 grain">
          {chapters.map((ch, i) => (
            <button
              key={`${ch.slug}-${i}`}
              ...
              className={`px-2 py-1.5 text-xs font-mono transition-colors ${
                i === chIndex
                  ...
                  : 'text-muted hover:bg-surface/80 hover:text-paper'
              }`}
            >
      ...
      {loading ? (
        ...
        <div className="space-y-1 bg-void p-1 border border-hairline">
          {pages.map((page, i) => (
            <img
              key={i}
              src={page.url}
        ...
        <div className="flex justify-center py-20 border border-hairline bg-surface/30 grain">
      ...
      <div className="flex justify-between gap-2 pt-4">
        <button
          ...
          disabled={chIndex === 0}
          className="flex-1 px-3 py-2 font-mono text-[10px] uppercase tracking-wider border border-hairline text-paper disabled:opacity-30 hover:border-amber/60 hover:text-amber transition-colors"
        >
          ← Prev
        </button>
        <button
